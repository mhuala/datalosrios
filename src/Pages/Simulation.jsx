...
export default function Simulation() {
  ...
  const handleSubmit = () => {
    ...
    let position_filtered = sectors.reduce(
      (acc, item, index) => (item.name === sectorSelected.name ? index : acc),
      []
    );
    {
      axios
                ...
                <div className="ml-5 p-10 flex flex-col justify-center max-w-2xl rounded bg-brand space-y-10">
                  ...
                  <div className="mt-4  text-gray-300 flex flex-col space-y-4">
                    <span className="text-lg lg:text-xl font-athiti">
                      Simula una variación en la demanda final de un sector
                      economico en la región y observa su efecto desagregado en
                      los otros sectores producto de los encadenamientos
                      productivos, cuantificando las nuevas diferencias en sus
                      demandas totales.
                    </span>
                    <a href="#" className="text-decoration text-blue-400">
                      → ¿Como se simula la inversión a nivel microterritorial?
                    </a>
                  </div>
                  {/* Inputs */}
                      ...
                      >
                        <span className="flex items-center space-x-5">
                          ...
                          <span className="pr-6 text-gray-100">
                            ¿Quieres ver los efectos?
                          </span>{" "}
                        </span>
                        <span className="pl-6 text-indigo-400"> SIMULA!</span>
                      </button>
                    </Link>
                  </div>
                </div>
        ...
        <div className="grid gap-4 grid-cols 1 lg:grid-cols-6 px-6 ">
          <div className="col-span-1 lg:col-span-2 px-10 bg-indigo-900 rounded-2xl">
            ...
            <p className="text-white text-lg lg:text-xl font-athiti text-start">
              En las siguientes tarjetas puedes ver el efecto desencadenado que
              implicaría varíar la demanda final del sector{" "}
              {sectorSelected.name} en ${ammount} millones de pesos,
              observandose la diferencia en la demanda final que provocaría
              además de que porcentaje de la demanda final total del año 2020
              representa esta variación. Esto es provocado por los
              encadenamientos productivos del sector que indican el nivel y tipo
              de interacción que existe entre los sectores.
            </p>

            <div className="h-4"></div>
            <p className="text-white text-lg lg:text-xl font-athiti text-start">
              El cálculo de estas diferencias en la demanda final se hace bajo
              la metodología de análisis de Matriz Insumo-Producto calculando la
              multiplicación del vector generado por esta nueva diferencia en la
              demanda del sector {sectorSelected.name} con la inversa de
              Leontief del 2020 (último año disponible).
            </p>
              ...
              {infoData.map((sector) => (
                <div>
                  <div className="p-5 bg-white rounded shadow-sm">
                    <div className="text-gray-400 text-md">{sector.x}</div>
                    <div className="flex items-center pt-1 justify-between">
                      ...
                      <span className="flex items-center px-2 py-0.5 mx-2 text-sm text-green-600 bg-green-100 rounded-full">
                        ...
                        <span>{(sector.z * 100).toFixed(6)}%</span>
                      </span>
                    </div>
                  </div>
                </div>
